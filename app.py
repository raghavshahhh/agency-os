...
Uses CRMSystem for clients and pipeline
"""
...
# Add engine to path for CRMSystem
...
# Get pipeline stats from CRM
pipeline_value = sum(d.get("value", 0) for d in all_deals if d.get("status") != "won")
active_clients = len([c for c in clients if c.get("status") == "client"])
...
kpis = [
 ...
 ("💼", str(len(all_deals)), "Pipeline Deals"),
...
with col_left:
 ...
 # Recent CRM deals
 for deal in sorted(all_deals, key=lambda x: x.get("created_at", ""), reverse=True)[:2]:
  activities.append({
   "icon": "🔵",
   "text": f"Deal: {deal.get('title', '')[:50]}",
